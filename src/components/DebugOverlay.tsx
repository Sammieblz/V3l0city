  ...
  return (
    <TouchableOpacity onPress={toggle} activeOpacity={0.9}>
      <View style={styles.container}>
        ...
        <Text style={styles.line}>heading source: {state.headingSource}</Text>
        <Text style={styles.line}>
          heading quality: {state.headingQuality}
        </Text>
        <Text style={styles.line}>
          heading accuracy:{' '}
          {state.headingAccuracyDegrees == null
            ? 'null'
            : state.headingAccuracyDegrees.toFixed(1)}
        </Text>
        <Text style={styles.line}>
          heading reasons: {state.headingReasons.join(', ') || 'none'}
        </Text>
        <Text style={styles.line}>
          quality score: {state.qualityScore.toFixed(2)}
        </Text>
        <Text style={styles.line}>
          reasons: {state.qualityReasons.join(', ') || 'none'}
        </Text>
